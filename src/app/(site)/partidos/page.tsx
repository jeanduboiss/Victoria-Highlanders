...
function MatchRow({ match, locale, t }: { match: PublicMatchFull, locale: string, t: any }) {
    ...
    <div className="grid grid-cols-[56px_1fr_auto] sm:grid-cols-[56px_1fr_120px_auto] items-center gap-4 py-4 px-4 sm:px-5 border-b border-white/[0.05] last:border-0 hover:bg-white/[0.02] transition-colors">
      ...
      <div className="min-w-0">
        ...
        <div className="flex items-center gap-2 mt-1 flex-wrap">
          ...
          {match.round && (
            <span className="text-[11px] text-white/20">{t('matchday')} {match.round}</span>
          )}
        </div>
      </div>
      ...
      <div>
        <span className={`font-sans text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full whitespace-nowrap ${isFinished
          ...
          {isFinished ? t('final') : isScheduled ? t('scheduled') : match.status}
        </span>
      </div>
    </div>
  )
...
export default async function PartidosPage() {
  ...
  const t = await getTranslations('subpages.matches')
  ...
  return (
    <div className="bg-[#0a0a0a] min-h-screen">
      <div className="mx-auto max-w-[1280px] px-4 lg:px-8 py-12 lg:py-16">
        <div className="mb-10">
          <p className="font-sans text-[11px] font-bold uppercase tracking-[0.25em] text-gold mb-2">{t('season')}</p>
        ...
        {matches.length === 0 || (hideResults && upcoming.length === 0) ? (
          <div className="flex h-60 items-center justify-center border border-white/5 bg-white/[0.02] rounded">
            <p className="font-sans text-[13px] font-bold tracking-widest uppercase text-gray-500">{t('noMatches')}</p>
          </div>
        ) : (
          <div className={`grid grid-cols-1 ${hideResults ? 'max-w-3xl mx-auto' : 'lg:grid-cols-2'} gap-8`}>
            {upcoming.length > 0 && (
              <div>
                ...
                <div className="bg-[#111] border border-white/[0.06] divide-y-0">
                  {upcoming.map((m) => <MatchRow key={m.id} match={m} locale={locale} t={t} />)}
                </div>
              </div>
            )}
            ...
            {finished.length > 0 && (
              <div>
                ...
                <div className="bg-[#111] border border-white/[0.06] divide-y-0">
                  {finished.map((m) => <MatchRow key={m.id} match={m} locale={locale} t={t} />)}
                </div>
              </div>
            )}
            ...
            {others.length > 0 && (
              <div className="lg:col-span-2">
                <div className="bg-[#111] border border-white/[0.06] divide-y-0">
                  {others.map((m) => <MatchRow key={m.id} match={m} locale={locale} t={t} />)}
                </div>
              </div>
            )}
          </div>
